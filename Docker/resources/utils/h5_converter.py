...
from resources.utils.fmc_utils import get_fmc_gradient_info, get_fmc_light_direction, get_fmc_metadata, compute_convex_image, get_centroids
...
def prepare_image_fmc(input_path, input_image, metadata, output_path=None, identifier='*.tif', descriptor='', normalize=[0,100],\
    ...
    if not use_itk:
        ...
        small_input_image = input_image[::downsampling_factor_z, ::downsampling_factor_xy, ::downsampling_factor_xy]

    # get sizes of original and low resolution images
    ...
    if get_lightmap:
        ...
        if use_itk:
            light_map_image = sitk.GetImageFromArray(1 - light_map_image.astype(np.uint16))
            light_map_image.SetSpacing(image_spacing_small)

            light_map_image = sitk.DanielssonDistanceMap(light_map_image, useImageSpacing=True)
            light_map_image = sitk.GetArrayFromImage(light_map_image).astype(np.uint16)

            # rescale distance image
            light_map_image = np.repeat(light_map_image, downsampling_factor_z, axis=0)
            light_map_image = np.repeat(light_map_image, downsampling_factor_xy, axis=1)
            light_map_image = np.repeat(light_map_image, downsampling_factor_xy, axis=2)
            dim_missmatch = np.array(input_image.shape)-np.array(light_map_image.shape)
            if dim_missmatch[0]<0: light_map_image = light_map_image[:dim_missmatch[0],...]
            if dim_missmatch[1]<0: light_map_image = light_map_image[:,:dim_missmatch[1],:]
            if dim_missmatch[2]<0: light_map_image = light_map_image[...,:dim_missmatch[2]]
    ...
    if get_surfacedistance:
        ...
        if use_itk:
            ...
            edt_image = np.repeat(edt_image, downsampling_factor_xy, axis=1)
            edt_image = np.repeat(edt_image, downsampling_factor_xy, axis=2)
            dim_missmatch = np.array(input_image.shape)-np.array(edt_image.shape)
            if dim_missmatch[0]<0: edt_image = edt_image[:dim_missmatch[0],...]
            if dim_missmatch[1]<0: edt_image = edt_image[:,:dim_missmatch[1],:]
            if dim_missmatch[2]<0: edt_image = edt_image[...,:dim_missmatch[2]]
